...
The [charge-only USB-C cable](http://www.apple.com/shop/question/answers/product/MLL82AM/A/is-this-a-thunderbolt-3-cable-or-just-a-usbc-cable/QT4CFYH92TCAK2242) is nicely rolled up like a snake and placed under the device. Putting that back after unrolling it, is virutally an impossible job. As there's no other space or cavity in the box, you're going to have to carry that around on your first day.
...
The keyboard is nice. It has a gentle click on start and end of a travel, but the return (enter) key could be horizontally aligned with shift, to reduce this annoying finger stretching. The keyboard backlighting looks smooth, but there was no budget left to give the Caps-lock key a the same backlight colour when switched on.
...
When you're working in a virtual machine with Windows, or running Windows applications where you're used to the F1-F12 keys, be sure to [enable the function keys for that application](http://www.imore.com/how-make-function-keys-default-touch-bar-display).
...
What with the missing headphone jack on the new iPhone, I'm very happy that the USB-C ports have not taken the place of a mini-jack, allowing me to plug in a normal headphone set and supporting the microphone right away.

## Charging this thing
Forgive the tone of the header, but I expect a computer not to behave like a phone or other thing with a battery. When OSX has died from a drained battery after failing to enter hibernate (sleep and persist RAM to disk) mode, I plug it in and want to start it up. However, due to [an issue that's been around for almost 10 years](https://superuser.com/q/216064) the battery first needs a bit of juice before the machine can boot up. After my first few days, this cost me some 20 minutes' time. Just imagine having to give a presentation first thing in the morning.
